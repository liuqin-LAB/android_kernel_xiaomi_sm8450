...
static int power_reset(struct fpc1020_data *fpc1020)
{
	int rc;
                              ...
                              char *buffer)
{
    ...
    mutex_lock(&fpc1020->lock);
    enabled = fpc1020->vreg ? regulator_is_enabled(fpc1020->vreg) : -ENODEV;
    mutex_unlock(&fpc1020->lock);
    if (enabled < 0)
        return enabled;
                              ...
                              const char *buf, size_t count)
{
    ...
    if (!strncmp(buf, "enable", strlen("enable"))) {
        enabled = regulator_is_enabled(fpc1020->vreg);
        if (enabled <= 0) {
            status = regulator_enable(fpc1020->vreg);
            usleep_range(100*1000, 200*1000);
        }
    } else if (!strncmp(buf, "disable", strlen("disable"))) {
        enabled = regulator_is_enabled(fpc1020->vreg);
        if (enabled > 0) {
            status = regulator_disable(fpc1020->vreg);
        }
    } else if (!strncmp(buf, "force_disable", strlen("force_disable"))) {
        status = regulator_force_disable(fpc1020->vreg);
    ...
    voltage = regulator_get_voltage(fpc1020->vreg);
    enabled = regulator_is_enabled(fpc1020->vreg);
...
static int device_prepare(struct fpc1020_data *fpc1020, bool enable)
{
	...
	if (enable && !fpc1020->prepared) {
        ...
        if (power_cfg == 1) {
            ...
            rc = regulator_is_enabled(fpc1020->vreg);
            if (rc < 0)
                goto exit;
            if (rc) {
                ...
                rc = regulator_set_load(fpc1020->vreg, 100000);
                if (rc < 0) {
            ...
            pr_info("fp_vdd_vreg is enabled %d!\n",regulator_get_voltage(fpc1020->vreg));
        } else {
